...
public class Caixa {
    ...
    public void imprimindoPedido(Item[] itens, int qtdPedido){
        ...
        for (int i=0; i < qtdPedido; i++) {
            ...
            System.out.println("QUANTIDADE " + itens[i].getQtdDoItem());
        }
        System.out.println("-----------------------------------------------------");
    }
    public void imprimindoProdutos(Produto[] produtos, int qtdProdutos){
        ...
        for (int i=0; i < qtdProdutos; i++) {
            ...
            System.out.println("QUANTIDADE EM ESTOQUE " + produtos[i].getQtdeEstoque());
        }
        System.out.println("-----------------------------------------------------");
    }
    public double buscandoPrecoItem(Produto[] produtos, Item[] itens, int qtdPedido, int qtdProdutos ) {
        ...
        for (int i=0; i < qtdPedido; i++) {
            for (int j=0; j < qtdProdutos; j++) {
                 if (Objects.equals(itens[i].getNome(), produtos[j].getNome())) {
                     preco = produtos[j].getPreco();
                     return preco;
                 }
            }
        }
